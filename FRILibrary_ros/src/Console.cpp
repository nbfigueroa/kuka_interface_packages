...
// Constructor 
//
Console_FRI::Console_FRI(		const unsigned int 	&Priority
...
// printf()
//
int Console_FRI::printf(const char* Format,...)
{
	int			Result;
...
void* Console_FRI::ConsoleThreadMain(void* ObjectPointer)
{
	unsigned int		i		=	0;

	Console_FRI* ThisObjectPtr = (Console_FRI*)ObjectPointer;
...
// flush()
//
void Console_FRI::flush(void) const
{
	fflush(this->Handler);
	return;
}
...
// GetFileHandler()
//
FILE* Console_FRI::GetFileHandler(void) const
{
	return(this->Handler);
}
